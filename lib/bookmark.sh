...
EOF
    ...
    # Escape all user-provided strings before using them in sed
    local safe_title; safe_title=$(_sed_escape "$title")
    local safe_uri; safe_uri=$(_sed_escape "$uri")
    ...
    local tags_yaml; tags_yaml=$(echo "$tags_input" | sed 's/ \{1,\}/, /g'); local tags_list; tags_list=$(echo "$tags_input" | sed 's/ \{1,\}/, /g')
    
    sed -e "s/{{ID}}/$id/g" \
        -e "s/{{TITLE}}/$safe_title/g" \
        -e "s/{{URI}}/$safe_uri/g" \
        -e "s/{{TAGS_YAML}}/$tags_yaml/g" \
        -e "s/{{TAGS_LIST}}/$tags_list/g" \
        -e "s/{{DATE}}/$date/g" \
        -e "s/{{DESCRIPTION}}/$safe_description/g" \
        "$template_file" > "$note_path"
        
    echo "Bookmark captured: $note_path"; _index_file "$note_path"
}