#!/usr/bin/env bash
function cmd_query() {
    local query_type="$1"; shift; local query="$*"; local results
    case "$query_type" in
        ...
        *) echo "Invalid query type. Use --tag, --alias, --type, or --fulltext." >&2; exit 1;;
    esac
    if [ -z "$results" ]; then echo "No results found."; return; fi
    
    # MODIFIED: Handle relative paths returned from the database.
    # The fzf preview must construct an absolute path.
    local choice
    choice=$(echo "$results" | fzf --preview "cat \"$ZETTEL_DIR/{}\"")
